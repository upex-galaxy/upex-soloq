...
const {
  ...
  SUPABASE_ANON_KEY = '', // Public anon key — required for all Supabase REST calls
...
> = {
  local: {
    ...
    api: 'http://localhost:3000', // Next.js app — domain API routes live at /api/*
    user: userCredentialsMap.local,
  },
  staging: {
    base: 'https://staging-upexsoloq.vercel.app',
    api: 'https://staging-upexsoloq.vercel.app', // Next.js app — domain API routes live at /api/*
...
export const config = {
  ...
  // Authentication config (Supabase endpoints - relative to apiUrl)
  auth: {
    loginEndpoint: '/auth/v1/token?grant_type=password',
    tokenEndpoint: '/auth/v1/token', // Substring to match for page.waitForResponse() interception
    meEndpoint: '/auth/v1/user',
    tokenLifetimeSeconds: 3600, // 1 hour (Supabase default)
    // Storage paths for authenticated sessions
    storageStatePath: '.auth/user.json',
    apiStatePath: '.auth/api-state.json',
  },

  // Supabase configuration (used by AuthApi for direct auth calls)
  supabase: {
    url: 'https://czuusjchqpgvanvbdrnz.supabase.co',
    anonKey: SUPABASE_ANON_KEY, // Public anon key — required as 'apikey' header for direct Supabase REST calls
  },
