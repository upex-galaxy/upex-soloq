...
|--------|-------|
| **Name** | SoloQ |
| **Type** | SaaS - Invoicing platform for LATAM freelancers |
| **Stack** | Next.js 16 App Router + Supabase (PostgreSQL + Auth) + Tailwind CSS 4 |
| **Backend** | Supabase REST API (`https://czuusjchqpgvanvbdrnz.supabase.co`) |
| **Staging URL** | `https://staging-upexsoloq.vercel.app` |
| **Jira Project** | `SQ` (ATC IDs: `SQ-XXX`) |
| **Test Framework** | Playwright + KATA + Allure |

**Auth Flow:**
```
POST /auth/v1/token?grant_type=password  →  { access_token, token_type: 'bearer', ... }
GET  /auth/v1/user                       →  SupabaseUser (flat object)
All requests require: apikey header (anon key)
```

**Main Entities:** clients, invoices, invoice_items, payments, payment_methods, business_profiles
