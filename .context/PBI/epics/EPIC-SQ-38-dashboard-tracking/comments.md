...
### Joel Armando Ramírez Rodríguez - 16/3/2026, 09:37:26
...
- Tabla principal `invoices` (status, due*date, total, paid*at, created_at).
- Join con `clients` para busqueda por cliente y render de columnas.
- Agregaciones por status/periodo mensual y calculo de overdue (`status='sent' AND due*date < CURRENT*DATE`).
...
***Fecha:**** 2026-03-28 ****QA Lead:**** Fernando Javier Masci ****Epic Jira Key:**** SQ-38 ****Status:*** Draft
...
***Data Flow:*** ```text User -> Dashboard UI -> GET /api/invoices + GET /api/invoices/dashboard -> PostgreSQL ^ | (status/search/sort/page) ```
...
_Last sync: 2026-03-28T21:41:10.278Z_
