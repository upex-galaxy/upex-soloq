...
[View in Jira](https://upexgalaxy65.atlassian.net/browse/SQ-29)

---

### Ximena Quintana - 2/3/2026, 12:42:25 PM
...
### Ximena Quintana - 2/3/2026, 6:38:27 PM
...
- ***Given:***
  - Usuario tiene `business*profile.default*terms` configurado (ej: `"Pago a 30 dias. Transferencia bancaria."`)
...
- Usuario autenticado
- `business*profile.default*terms` configurado
...
### Automation for Jira - 2/8/2026, 1:43:18 AM

🔎 Pull Request created. Task is pending to ANALYZE and REVIEW by the team. Waiting for PR Approval.

---

### Automation for Jira - 2/8/2026, 1:44:27 AM

✅ Pull Request is successfully MERGED. Task is Done.

---

### Ely - 2/8/2026, 1:44:56 AM

Feature implementada y desplegada en staging.

***PR:*** [#34](https://github.com/upex-galaxy/upex-soloq/pull/34)
***Branch:*** feat/[https://upexgalaxy65.atlassian.net/browse/SQ-29#icft=SQ-29](https://upexgalaxy65.atlassian.net/browse/SQ-29#icft=SQ-29)/add-notes-terms-invoice

***Cambios implementados:***

- Campo de notas con contador de caracteres (max 2000)
- Campo de términos y condiciones con contador (max 1000)
- Pre-llenado de términos desde perfil de negocio (default_terms)
- Ambos campos opcionales

***Columnas DB agregadas:***

- `invoices.terms` (text, nullable)
- `business*profiles.default*terms` (text, nullable)

@Ximena Quintana La funcionalidad está lista para pruebas en el ambiente de staging.

---

### Ximena Quintana - 2/10/2026, 12:58:23 PM

## QA Exploratory Testing (UI + Network) - Ejecutado en Staging

**Fecha:** 2026-02-10  
**Ambiente:** [https://staging-upexsoloq.vercel.app/invoices/create](https://staging-upexsoloq.vercel.app/invoices/create)  
**Enfoque:** Solo casos UI con validación por Network 

### Test Cases ejecutados

| ***ID**** | ****Test case**** | ****Resultado esperado**** | ****Resultado actual**** | ****Estado*** |
| --- | --- | --- | --- | --- |
| TC-01 | Guardar **notes** y **terms** al crear factura | Al guardar borrador, la API crea factura y persiste ambos campos | **POST /api/invoices** respondió **201**; payload incluyó **notes** y **terms** con valores enviados | PASSED |
| TC-02 | Verificar estructura de payload al guardar factura | Request body debe incluir **clientId, dueDate, taxRate, notes, terms** | Keys capturadas: **[clientId, dueDate, notes, taxRate, terms]** | PASSED |
| TC-03 | Límite de caracteres en **notes** (max+1) | No debe exceder el máximo; contador visible | Se intentó 2001 chars, quedó en 2000; contador **2000/2000** | PASSED |
| TC-04 | Límite de caracteres en **terms** (max+1) | No debe exceder el máximo; contador visible | Se intentó 1001 chars, quedó en 1000; contador **1000/1000** | PASSED |
| TC-05 | Guardar factura con **notes** y **terms** vacíos | Debe permitir guardar (campos opcionales) | **POST /api/invoices** = **201**; payload envió **notes: ""** y **terms: ""** | PASSED |
| TC-06 | Prefill de **terms** desde **business*profile.default*terms** | En nueva factura, **terms** debe venir prellenado si existe default | No validable por error de backend en fetch de perfil de negocio (**406**) | FAILED/BLOQUEADO |
| TC-07 | Sanitización HTML/script en **notes** | Debe tratarse/renderizarse seguro como texto plano | Se guardó con **[script]alert(1)**[**/script**](/script) (201), pero no hay preview/PDF habilitado para validar render final | PARTIAL |
| TC-08 | No mostrar secciones vacías en preview/PDF | Si **notes/terms** vacíos, no se debe mostrar sección ni espacio extra | No ejecutable: módulo de visualización aún **Próximamente** | BLOQUEADO |
| TC-09 | Prefill solo en creación (no sobrescribir en edición) | En edición no debe reaplicar default sobre valor guardado | No ejecutable: flujo de edición no disponible | BLOQUEADO |
| TC-10 | Notes al final del invoice/PDF con saltos de línea | Debe renderizarse al final respetando formato | No ejecutable: preview/PDF no disponible en flujo actual | BLOQUEADO |
| TC-11 | Caracteres especiales/emojis en PDF | Deben renderizar correctamente (si permitido) | No ejecutable: preview/PDF no disponible | BLOQUEADO |

### Requests observados en Network

- ***POST /auth/v1/token?grant_type=password*** → 200
- ***GET /api/clients?sortBy=name&sortOrder=asc&limit=100*** → 200
...
- Cobertura de creación + persistencia de ***notes/terms*** validada ✅
- Límites de caracteres (hard limit) validados ✅
- Quedan escenarios bloqueados por falta de UI de edición/preview y por ***406**** en ****business_profiles*** ⚠️

---

### Ximena Quintana - 2/10/2026, 12:59:32 PM

Bug identificado durante exploratory testing de [https://upexgalaxy65.atlassian.net/browse/SQ-29#icft=SQ-29](https://upexgalaxy65.atlassian.net/browse/SQ-29#icft=SQ-29).

---

### Ximena Quintana - 2/10/2026, 12:59:33 PM
...
_Last sync: 2026-03-02T19:53:52.855Z_
